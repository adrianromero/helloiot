...
public class TopicInfoGaugeNode implements TopicInfoNode {
    ...
    @FXML
    public ChoiceBox<GaugeNodeFormat> editformat;    
    @FXML
    public TextField editmin;
    @FXML
    public TextField editmax;
    @FXML
    public ChoiceBox<GaugeType> editgaugetype;       
    @FXML
    public ColorPicker editbarcolor;
    @FXML
    Button clearbarcolor;
    @FXML
    ...
    public TopicInfoGaugeNode() {
        FXMLNames.load(this, "com/adr/helloiot/fxml/topicinfogaugenode");            
    }
    ...
    public void initialize() {
        ...
        editformat.setItems(FXCollections.observableArrayList(GaugeNodeFormat.values()));
        editformat.getSelectionModel().clearSelection();
        editformat.setConverter(new StringConverter<GaugeNodeFormat>() {
            @Override
            public String toString(GaugeNodeFormat editnodeformat) {
                return resources.getString("format." + editnodeformat.name());
            }
            @Override
            public GaugeNodeFormat fromString(String value) {
                return null;
            }
        });
        editformat.valueProperty().addListener((ObservableValue<? extends GaugeNodeFormat> ov, GaugeNodeFormat old_val, GaugeNodeFormat new_val) -> {
            updateCurrentTopic();
        });
        ...
        editmin.textProperty().addListener((ObservableValue<? extends String> ov, String old_val, String new_val) -> {
            updateCurrentTopic();
        });

        editmax.textProperty().addListener((ObservableValue<? extends String> ov, String old_val, String new_val) -> {
            updateCurrentTopic();
        });
        
        editgaugetype.setItems(FXCollections.observableArrayList(GaugeType.values()));
        editgaugetype.getSelectionModel().clearSelection();
        editgaugetype.setConverter(new StringConverter<GaugeType>() {
            @Override
            public String toString(GaugeType editnodeformat) {
                return resources.getString("gaugetype." + editnodeformat.name());
            }
            @Override
            public GaugeType fromString(String value) {
                return null;
            }
        });
        editgaugetype.valueProperty().addListener((ObservableValue<? extends GaugeType> ov, GaugeType old_val, GaugeType new_val) -> {
            updateCurrentTopic();
        });        

        clearbarcolor.setGraphic(IconBuilder.create(IconFontGlyph.FA_SOLID_TRASH_ALT, 14.0).styleClass("icon-fill").build());
        editbarcolor.setValue(null);
        editbarcolor.valueProperty().addListener((ObservableValue<? extends Color> observable, Color oldValue, Color newValue) -> {
            updateCurrentTopic();
        });
    ...
    @FXML
    void onClearBarColor(ActionEvent event) {
        editbarcolor.setValue(null);
    }
